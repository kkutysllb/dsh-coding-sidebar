 ...
 */
import { CLOSE_FENCE_RE, OPEN_FENCE_RE, fenceInfo } from './mermaid-blocks.mjs';
/**
 ...
 */
export function analyzeHtmlSegment(source) {
    ...
    for (let index = 0; index < tokens.length; index += 1) {
        ...
        for (let depth = stack.length - 1; depth >= 0; depth -= 1) {
            if (stack[depth].token.tag === token.tag) {
                matchAt = depth;
                break;
            }
        }
    ...
    const structural = [];
    for (const token of unmatchedCloses)
        structural.push({ token, kind: 'close' });
    for (const entry of stack)
        structural.push({ token: entry.token, kind: 'open' });
    structural.sort((a, b) => a.token.start - b.token.start);
    if (structural.length === 0)
        return { parts: [{ kind: 'html', html: source }] };
    const parts = [];
    let cursor = 0;
    for (const { token, kind } of structural) {
        if (token.start > cursor)
            parts.push({ kind: 'html', html: source.slice(cursor, token.start) });
    ...
    if (cursor < source.length)
        parts.push({ kind: 'html', html: source.slice(cursor) });
    return { parts };
}
/**
...
export function splitHtmlBlocks(text) {
    ...
    const flushMarkdown = () => {
        ...
        while (markdown.length > 0 && isBlank(markdown[markdown.length - 1] ?? ''))
            markdown.pop();
    ...
    for (const line of lines) {
        ...
        if (isBlank(line) && markdown.length === 0)
            continue;
        markdown.push(line);
    }
    flushMarkdown();
 ...
 */
export function collectReferenceDefinitions(segments) {
    const defs = [];
    for (const segment of segments) {
        if (segment.kind !== 'markdown')
            continue;
        for (const line of segment.text.split('\n')) {
            if (REFERENCE_DEF_RE.test(line))
                defs.push(line);
        }
    }
    return defs.join('\n');
 ...
 */
export function analyzeMarkdownHtml(text) {
