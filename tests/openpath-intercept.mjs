 ...
 */
export function isFolderRevealPath(path) {
    if (path === '.' || path === './')
        return true;
 ...
 */
export function wrapOpenPath(workspaces, deps) {
    ...
    workspaces.openPath = (path) => {
        if (deps.takeoverEnabled()) {
            ...
            if (sessionId !== undefined) {
                if (isFolderRevealPath(path))
                    deps.revealInExplorer(path, sessionId);
                else
 ...
 */
export function wrapRemoteOpenPath(session, deps) {
        ...
        get: function () {
            ...
            return (request) => {
                if (deps.takeoverEnabled()) {
                    const sessionId = deps.currentSessionId();
                    if (sessionId !== undefined) {
                        if (isFolderRevealPath(request.path))
                            deps.revealInExplorer(request.path, sessionId);
                        else
 ...
 */
export function browserUrlOfOpen(options) {
 ...
 * @returns the disposer restoring the original method (HMR-safe).
 */
export function wrapNativeBrowserOpen(right, open) {
 ...
 */
export function fileTargetOfAddress(address) {
 ...
 */
export function wrapSidebarRight(right, deps) {
    ...
    right.openResource = function (address, options) {
        ...
        if (target !== undefined && sessionId !== undefined) {
            ...
            if (target.path === '' || isFolderRevealPath(target.path))
                deps.revealInExplorer(target.path, sessionId);
            else
                deps.openInSidebar(target.path, sessionId);
            return;
        }
