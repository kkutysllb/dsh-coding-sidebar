 ...
 */
export function resolveLocalMediaDest(dest, scope, filePath, origin) {
...
export function rewriteLocalImageUrls(text, scope, filePath, origin) {
    ...
    const refsRewritten = inline.replace(/^(\s*\[([^\]]+)\]:\s*)(<[^>]+>|[^\s]+)/gm, (match, head, label, dest) => {
        if (!imageLabels.has(label.toLowerCase()))
            return match;
        return `${head}${resolve(dest.replace(/^<|>$/g, ''))}`;
    });
    return refsRewritten.replace(/\u0000(\d+)\u0000/g, (_m, index) => masks[Number(index)] ?? '');
}
