		...
		* membership before using it as a command cwd. */
		...
		const api = {
			...
			*/
			...
			sidechatDebug: (text) => call("sidechat.debug", { text }),
		...
		*/
		...
		function SideChatView(props) {
			...
			*/
			...
			const debugOnce = (0, react.useRef)(false);
			const debugCount = (0, react.useRef)(0);
			const debugLive = (0, react.useRef)(0);
			/**
			...
			*  @returns whether the merged transcript grew (the poll's pacing signal). */
			const fetchThread = (0, react.useCallback)(async (childId) => {
				...
				try {
					...
					if (debugCount.current < 3) {
						...
						api.sidechatDebug(`poll ok thread=${childId} entries=${cache.entries.length} live=${liveRef.current.length}`);
					} else if (liveRef.current.length > 0 && debugLive.current < 20) {
						debugLive.current += 1;
						api.sidechatDebug(`poll LIVE thread=${childId} entries=${cache.entries.length} live=${liveRef.current.length}`);
					}
					...
					if (!controller.signal.aborted) {
						...
						if (debugCount.current < 3) {
							debugCount.current += 1;
							api.sidechatDebug(`poll ERROR thread=${childId} ${message}`);
						}
						setError(message);
					}
					return false;
			...
			(0, react.useEffect)(() => {
				if (!visible || threadId === void 0) {
					if (threadId !== void 0 && !debugOnce.current) {
						debugOnce.current = true;
						api.sidechatDebug(`poll skipped thread=${threadId} visible=${String(visible)}`);
					}
					return;
				}
				if (!debugOnce.current) {
					debugOnce.current = true;
					api.sidechatDebug(`poll start thread=${threadId} visible=${String(visible)}`);
				}
