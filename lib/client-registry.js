		...
		*/
		...
		function renderRow(row, labels) {
			switch (row.kind) {
				...
				case "turnSummary": {
					const parts = [];
					if (row.inputTokens !== void 0 || row.outputTokens !== void 0) parts.push(`${formatTokens(row.inputTokens ?? 0)} → ${formatTokens(row.outputTokens ?? 0)}`);
