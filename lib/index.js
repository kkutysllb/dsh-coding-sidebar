...
*/
function installAgentModelSelection(ctx, agent) {
	...
	try {
		...
		ctx.logger?.warn(`[dsh-coding-sidebar] side chat: model selection was not installed for ${agent.session.id}: ${error instanceof Error ? error.message : String(error)}`);
		return false;
	}
}
...
function readSessionModelSelection(ctx, session) {
	...
	if (typeof projections?.stateOf !== "function") return void 0;
	try {
...
*/
function installedSelectionOf(ctx, agent) {
	const agents = ctx.get("agents");
	if (typeof agents?.selectionFor !== "function") return void 0;
	try {
...
function alignThreadModelToParent(ctx, agent) {
	...
	const parentSession = ctx.get("sessions")?.get?.(parentSessionId);
	if (parentSession === void 0 || parentSession === null) return;
	...
	try {
		...
		ctx.logger?.warn(`[dsh-coding-sidebar] side chat: could not align ${agent.session.id} to the parent model: ${error instanceof Error ? error.message : String(error)}`);
	}
}
...
async function composePersistedSetup(ctx, childId) {
	const persistence = ctx.get("sessionPersistence");
	if (persistence === void 0) return (_agentCtx, agent) => {
		installAgentModelSelection(ctx, agent);
	};
	...
	await handle.close();
	const presets = ctx.get("agentPresets");
	if (presets === void 0 || presetId === void 0) return (_agentCtx, agent) => {
		installAgentModelSelection(ctx, agent);
	};
	const resolved = await presets.resolve(presetId);
