...
function alignThreadModelToParent(ctx, agent) {
	...
	if (parentSession === void 0 || parentSession === null) {
	...
	try {
		...
		ctx.logger?.info?.(`[dsh-coding-sidebar] side chat ${agent.session.id} follows the parent model: ${installed?.provider ?? "?"}/${installed?.model ?? "?"} → ${target.provider}/${target.model}`);
	} catch (error) {
