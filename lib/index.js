...
import { appendFileSync, chmodSync, createReadStream, createWriteStream, existsSync, readFileSync, realpathSync } from "node:fs";
...
function diagnose$1(text) {
	try {
		appendFileSync("/tmp/dsh-sidechat-debug.log", `${(/* @__PURE__ */ new Date()).toISOString()} live ${text}\n`);
	} catch {}
}
...
var AssistantLiveBuffer = class {
	...
	constructor(ctx) {
		...
		this.attach(host, "ctx", (payload) => {
			...
			if (this.frames <= 12 || this.frames % 50 === 0) {
				...
				diagnose$1(`frame #${this.frames} session=${String(agent?.session?.id)} type=${String(frame?.type)} index=${String(frame?.index)}`);
			}
			this.accept(payload);
		});
		const root = ctx.root;
		if (root !== void 0 && root !== ctx) this.attach(root, "root", (payload) => {
			this.frames += 1;
			if (this.frames <= 12 || this.frames % 50 === 0) {
				const frame = payload.frame;
				const agent = payload.agent;
				diagnose$1(`frame#${this.frames} via=root session=${String(agent?.session?.id)} type=${String(frame?.type)} index=${String(frame?.index)}`);
			}
			this.accept(payload);
		});
		else diagnose$1("root channel unavailable (ctx.root missing or identical)");
	...
	*/
	...
	/** 挂一条全局监听（诊断留痕记录通道名）。 */
	attach(host, via, listener) {
		try {
			host.on("agent/assistant-stream", listener, { global: true });
			diagnose$1(`subscribed agent/assistant-stream via=${via} (global)`);
		} catch (cause) {
			diagnose$1(`subscribe FAILED via=${via} ${cause instanceof Error ? cause.message : String(cause)}`);
		}
	}
	/** 只记第一次的丢弃原因（诊断）。 */
	note(reason) {
		if (this.dropped !== void 0) return;
		this.dropped = reason;
		diagnose$1(reason);
	}
	/** 折叠一帧。 */
...
/** 诊断留痕（定位后删）：把一行文本追加到 /tmp 日志。 */
function diagnose(_ctx, text) {
	try {
		appendFileSync("/tmp/dsh-sidechat-debug.log", `${(/* @__PURE__ */ new Date()).toISOString()} host ${text}\n`);
	} catch {}
}
/** 读一个可选的非负整数负载字段。 */
...
function buildSidechatApi(ctx) {
	...
	const eventsOf = async (childId, payload) => {
		...
		const live0 = liveThreadAgent(ctx, childId) !== void 0;
	...
	return {
		...
		"sidechat.debug": async (payload) => {
			try {
				const { appendFileSync } = await import("node:fs");
				const text = typeof payload === "object" && payload !== null ? String(payload.text ?? "") : "";
				appendFileSync("/tmp/dsh-sidechat-debug.log", `${(/* @__PURE__ */ new Date()).toISOString()} client ${text}\n`);
			} catch {}
			return { accepted: true };
		},
		"sidechat.events": async (payload) => {
			...
			try {
				...
				try {
					const { appendFileSync } = await import("node:fs");
					appendFileSync("/tmp/dsh-sidechat-debug.log", `${(/* @__PURE__ */ new Date()).toISOString()} events child=${childId} ERROR=${cause instanceof Error ? `${cause.name}: ${cause.message}` : String(cause)}\n`);
				} catch {}
				throw cause;
			}
		},
